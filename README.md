...
小爷做私域二十年，从 0 带过好几个盘子到过亿。风物执守把这些年真正管用的判断，拆成 **26,895 条能查、能引用的知识原子**和 **43 个能直接调的 Skill**——每条判断都能落到具体的原子编号上；实在查不到依据，也直说"库里没有实证"，不硬编。

先说清楚一个前提：**私域本身就能做流量。** 别学市面上"先做流量、再做私域"那一套——朋友圈就是广告位，老客就是转介绍源，内容和导流本来就是私域自己的本事。
...
/xy-vault 把这个文件夹变成我的生意知识库，以后直接从里面查。
```
...
| 千川一直亏，要不要停 | 投流该不该做、止损纪律并入生意基本盘诊断 |
...
`knowledge/atoms.jsonl` 里 26,895 条知识原子，按 13 个方向、94 个细分块打了标签，条条能独立成立、能互相回链。来源分三块：小爷自己的实战（课程体系、真实账号和直播复盘、二十年创业史，带真实数字的都标 `high` 置信）；商业管理谈判这类方法论（全部重写成生意场景的话再入库）；抖音小红书微信视频号四大平台的现行规则（2026-08 查证过，标了失效风险）。检索排序上，自有实战永远排前面。
...
本仓库用 [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/) 授权（见 `LICENSE`）：个人使用、学习、研究和非商业项目可以直接拿去用；公开发布衍生作品时注明来源；商业用途要单独授权，联系方式见上面。
