...
这是 `/xy-mp-layout` 的样式源。15 个风格的 CSS 都在这里，生成 HTML 时按 style id 来取。

先说清一件事：下面的 CSS 是**设计稿**，不是可以直接塞进 `<style>` 的成品。它只负责告诉你每个风格的字体、字号、层级、颜色、边框和间距长什么样；真正交付的 HTML 里，这些属性要按 `../SKILL.md` 的「粘贴兼容 6 条」摊到每一个可见元素的 `style` 属性上。

## 从设计稿到成品

生成正式交付 HTML 时：

1. `body` 上的字体、字号、行高、颜色，逐个补到每一个正文元素上；`body` 自己只留预览用的宽度和内外边距，正文不靠它继承。
2. `h1` `h2` `h3` `p` `blockquote` `ul` `ol` `li` `strong` `code` `pre` `hr` 的规则，展开成对应元素的 `style` 属性。
3. 文稿开头的首个一级标题默认只进 `<title>`，不进正文——所以 `h1` 的规则通常只在用户明确要求正文保留标题时才用得上。
4. class、id、`:before`、`:after` 全删。伪元素做的装饰，改成稳定的边框、留白或真实元素；没必要就砍掉。
5. 单色 `background` 改写为 `background-color`。
6. 只在浏览器预览里成立、粘进公众号就消失的样式，一律不用。

总览页是本地预览页，不进公众号，可以照常用 `<style>` 和 class。
